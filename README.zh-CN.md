...
- 多种比例：文生图 `1:1`、`4:3`、`3:4`、`16:9`、`9:16`、`3:2`、`2:3`、`21:9`;图生图另有 `match_input_image`(默认,跟随首张参考图)
- 分辨率：`1K`(约 200 万像素)、`2K`(约 400 万像素),按档按张计费;参考图 1-10 张,支持 JPG/PNG/WebP,不支持 SVG
- 本地输出：图片会保存到 `outputs/`
...
> 使用 `$hiapi-seedream-5-pro` 生成一张海面日落的 16:9 图片。
