...
脚本会先把现有正式目录或旧目录备份到 `~/.cache/hiapi-skill-backup/`，再安装当前版本；已有 `.env` 会保留，同时会检查 `HIAPI_API_KEY` 是否已设置。独立 CLI 从进程环境读取 `HIAPI_API_KEY`；保留 `.env` 是为了避免使用该文件的 Agent 运行时或封装脚本丢失配置。
...
手动升级时，请先把现有 `hiapi-seedream-5-0-pro` 和旧版 `hiapi-seedream-5-pro` 目录移出 skills 目录。直接克隆到最终路径时，如果目标仍存在会安全失败；推荐安装器会自动完成迁移、备份和回滚。
...
`success` 和 `fail` 都可能触发终态回调。服务端需要按 `taskId` 幂等处理。若要启用签名，请先在 HiAPI 账号设置页配置 16-256 字符的 Webhook 签名密钥；未配置时回调不会带签名头。验签时对原始请求体校验 `X-HiAPI-Timestamp` 与 `X-HiAPI-Signature`，采用常量时间比较，并拒绝与当前时间相差超过 5 分钟的时间戳。本地调试或回调失败补偿再使用 `GET /v1/tasks/:id`。
